...
test_transform.transforms.append(transforms.Resize((128,128)))
test_transform.transforms.append(transforms.ToTensor())

if dataset == 'zhang':
    test_data = Zhang('zhanglab/train', train=True, transform = test_transform, img_size=(128, 128))
else:
    test_data = ChexPert('chexpert/train_256_pa', train=True, transform = test_transform, img_size=(128, 128))
...
def get_train_embeds(model, size, defect_type, transform, device):
    ...
    with torch.no_grad():
        for x in dataloader_train:
            embed, logit = model(x.to(device).expand((-1,3,-1,-1)))
...
def eval_model(modelname, defect_type, device="cpu", save_plots=False, size=256, show_training_data=True, model=None, train_embed=None, head_layer=8, density=GaussianDensityTorch()):
    ...
    if test_data_eval is None:# or cached_type != defect_type:
        ...
        #test_data_eval = MVTecAT("data", 'toothbrush', size, transform = test_transform, mode="test")
        #test_data = Zhang('/media/administrator/1305D8BDB8D46DEE/jhu'+'/ZhangLabData/CellData/chest_xray/val', train=False, full=True, transform = transform, img_size=(128, 128))

        if dataset == 'zhang':
            test_data_eval = Zhang('zhanglab/val', train=False, full=True, transform = test_transform, img_size=(128, 128))
        else:
            test_data_eval = ChexPert('chexpert/val_256_pa', train=False, full=True, transform = test_transform, img_size=(128, 128))
    ...
    roc_auc, results = plot_roc(labels, distances, eval_dir / "roc_plot.png", modelname=modelname, save_plots=save_plots)
...
if __name__ == '__main__':
    ...
    # find models
    #model_names = [list(Path(args.model_dir).glob(f"best"))[0] for data_type in types if len(list(Path(args.model_dir).glob(f"model-{data_type}*"))) > 0]
    ...
    if dataset == 'zhang':
        test_data_eval = Zhang('zhanglab/test', train=False, full=True, transform = test_transform, img_size=(128, 128))
        roc_auc, results = eval_model('models/'+dataset+'_best.tch', 'none', save_plots=args.save_plots, device=device, head_layer=args.head_layer, density=density())
    else:
        test_data_eval = ChexPert('chexpert/our_test_256_pa', train=False, full=True, transform = test_transform, img_size=(128, 128))
        roc_auc, results = eval_model('models/'+dataset+'_best.tch', 'none', save_plots=args.save_plots, device=device, head_layer=args.head_layer, density=density())
