...
def run_training(data_type="screw",
    ...
    model_name = f"model" + '-{date:%Y-%m-%d_%H_%M_%S}'.format(date=datetime.datetime.now() )
    ...
    #train_data = MVTecAT("data", 'toothbrush', transform = train_transform, size=int(size * (1/min_scale)))
    
    if dataset == 'zhang':
        train_data = Zhang('zhanglab/train', train=True, transform = train_transform, img_size=(128, 128))
    else:
        train_data = ChexPert('chexpert/train_256_pa', train=True, transform = train_transform, img_size=(128, 128))

    dataloader = DataLoader(Repeat(train_data, 3000), batch_size=batch_size, drop_last=True,
                            shuffle=True, num_workers=0, collate_fn=cut_paste_collate_fn,
                            pin_memory=True)

    
    # Writer will output to ./runs/ directory by default
    #riter = SummaryWriter(Path("logdirs") / model_name)
    ...
    for step in tqdm(range(epochs)):
        ...
        batch_embeds = []
        #batch_idx, (data, _) = next(dataloader_inf)
        batch_idx, data = next(dataloader_inf)
        xs = [x.to(device)for x in data]
        #xs = [x.to(device).unsqueeze(1) for x in data]

        # zero the parameter gradients
        optimizer.zero_grad()
        #print()
        xc = torch.cat(xs, axis=0).expand((-1,3,-1,-1))
        #xc = torch.cat(xs, axis=0).expand((-1,3,-1,-1))
        #print(len(xs), xs[0].shape, xc.shape, '???')
...
#         loss = loss_fn(ip, torch.arange(0,x1.size(0), device=device))
        ...
        #print(y.shape, xs[0].size(0))
        #y = y.repeat_interleave(3).view(logits.shape[0],-1)
...
#         print(predicted)
#         print(y)
        accuracy = torch.sum(predicted==y) / predicted.size(0)
        ...
        if test_epochs > 0 and epoch % test_epochs == 0:
            ...
            model.eval()
            roc_auc, results = eval_model(model_name, 'null', device=device,
                                save_plots=False,
                                size=size,
            ...
            if roc_auc >= best_auc:
                torch.save(model.state_dict(), model_dir / dataset+"_best.tch")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--no-pretrained', dest='pretrained', default=False, action='store_false',
